...
export function DailyChallenge() {
  ...
  const handleWordSubmit = async (word: string) => {
    ...
    // Validate against engine (dictionary + rules)
    const validation = await validateWord(word, words)
    if (!validation.isValid) {
      setError(validation.errors?.[0]?.message || "Invalid word")
      return
    }
    ...
    // Daily rules handled by engine validation via game mode manager
  ...
  const handleUsePowerUp = (type: PowerUpType) => {
    switch (type) {
      ...
      case "warp":
        ...
        break
      case "flip":
        setError("Flip used (engine integration handled via powerUpSystem)")
        break
      case "bridge":
