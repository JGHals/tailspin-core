...
import { validateWord as engineValidateWord } from "@/validation/word-validation"
...
export const WordInput: FC<WordInputProps> = ({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Validate with engine (includes dictionary + chain checks when provided)
    const validation = await engineValidateWord(value, lastWord ? [lastWord] : [])
    if (!validation.isValid) {
      // Defer to parent to show error; do not submit invalid input
      return
    }
