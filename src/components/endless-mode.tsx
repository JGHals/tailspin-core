...
export function EndlessMode() {
  ...
  const startGame = async (startWord?: string) => {
    const manager = new GameModeManagerImpl('endless')
    if (user) manager.setUserId(user.id)
    await manager.startGame({ startWord })
    ...
    setError(null)
    ;(window as any).__endlessManager = manager
  }
  ...
  const handleWordSubmit = async (word: string) => {
    ...
    const validation = await validateWord(word, words)
    if (!validation.isValid) {
      setError(validation.errors?.[0]?.message || "Not a valid word")
      return
    }
    ...
    setWords(newWords)
    setScore(score + (validation.stats?.length || word.length))
    setMoves(moves + 1)

    if (validation.isTerminalWord) {
      const bonus = 20 + Math.max(0, word.length - 5) * 2
      setCurrentTerminalWord(word)
      setTerminalBonus(bonus)
      setIsNewTerminalDiscovery(true)
      setScore(score + (validation.stats?.length || word.length) + bonus)
